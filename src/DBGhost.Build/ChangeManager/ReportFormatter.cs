...
namespace DbGhost.Build.ChangeManager
{
	public class ReportFormatter : IReportFormatter
	{
		...
		private Converter CreateConverter()
		{
			...
			{
				"DB Ghost ChangeManager",
				"DB Ghost Change Manager",
				"Innovartis.DBGhost.ChangeManagerCmd",
				"Running (DB Ghost Change Manager)"
			});
			Predicate<string> arg_2BD_2 = (string e) => (e.StartsWith("DB Ghost Change Manager") || e.StartsWith("Innovartis.DBGhost.ChangeManagerCmd")) && e.Contains("complete");
